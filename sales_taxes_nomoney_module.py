...
def get_taxes(item):
    ...
    if "imported" in item:
        adtax = 0.050
        item = item.replace('imported', '') 'remove "imported" from the list to make easier parsing of item
    else:
        adtax = 0.000
...
def print_receipt(n, quantity):
    ...
    for i in range(0, len(quantity),6):
        ...
        aux_value = float(quantity[i+2])
        print(quantity[i], quantity[i+1], ":", "{0:.2f}".format(aux_value))

    print("Sales Taxes: ", "{0:.2f}".format(sales_taxes))
